 ...
 *
 * <p>业务侧可以用 MyBatis / JPA / JDBC / 其它任一方案实现；本工程不提供默认实现，
 * 只负责定义契约和表结构（见下方）。
 *
 * <h3>表结构 {@code zip_extract_record}</h3>
 * 用一张树形表同时表达"zip 的目录结构"和"每个文件在云存储的位置"。
 ...
 * <p>一些设计说明：
 * <ul>
 *     <li>{@code (task_id, full_path)} 做唯一索引，天然处理"不同目录重名"：
 *         路径不同即 key 不同，落库互不冲突。</li>
 *     <li>只有 {@link EntryType#FILE} 节点有 {@code storage_key}；
 *         {@code DIRECTORY} / {@code NESTED_ZIP} 为 {@code NULL}，不占云存储。</li>
 *     <li>嵌套 zip 用 {@code !/} 分隔（和常见 Java 工具 URL 一致，便于 debug）；
 *         比如 {@code pack/nested.zip!/inner/a.txt}。</li>
 *     <li>不需要 {@code sort_order}：业务只要求目录结构完整，同层顺序不做保证。</li>
 * </ul>
 */
public interface ArchiveEntryRepository {
     ...
     * 批量插入。
     *
     * <p>契约：插入后可以回填 {@link ArchiveEntry#setId(Long)}，业务如不需要可忽略。
     * 建议在一个事务里完成整批插入。
     *
    ...
    /**
     * 按 taskId 查出全部条目，用于打包。返回顺序不做要求（打包器内部按
     * {@code parentPath / fullPath} 还原树结构）。
     *
     * @param taskId 任务ID
